...
- **自动合成（主）**：按配方自动合成，物品进入背包后自动检查；材料不足提示、充足自动合成；可选"保留一份原材料"。
- **自动回收（辅）**：指定物品自动拆解成废料（沿用游戏 Salvage 蓝图）；支持白名单/黑名单模式与"保底保留 1 个"。
- **自动修复**：耐久低于阈值自动修复主手/副手/背包物品；手持主副手时每 1 秒检测、未手持时每 5 秒；未手持时不提示（仍自动修复）；材料不足且手持时可提示、可切换空手防损坏。

## 快捷标记（Alt+左键）

- 背包界面：**Alt+左键**点击物品 = 标记/取消标记自动回收（写入 `ItemRules`）
- 合成界面：**Alt+左键**点击配方 = 标记/取消标记自动合成（写入 `BlueprintRules`）
- 改动立即写入配置并生效，绿色=已加入、黄色=已移出
